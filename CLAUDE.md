...
- Keep published explanation in the docstring and internal rationale in an ordinary comment. `schemas/json/*.json` is generated from **class docstrings** and ships to the client inside the OCI image, so a design argument written there is disclosed to every engagement. The test: a docstring says what the thing is and which values are valid; a comment says why we built it that way, what it replaced, and what is coming later.
